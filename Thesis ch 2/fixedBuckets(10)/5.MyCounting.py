...
with open('myNegMappingDict_221f', 'rb') as f:
    myNegMappingDict = pickle.load(f)
with open('positive_ism_to_genenames_221f.txt') as f:
...
for itemSetLine in itemSets:
    ...
    value1 = (int(support)/positive_count) * 100
    ...
    if len(listOfIndicesForItemSets) != 0:
        ...
        value2 = (len(intersection)/negative_count) * 100
    else:
        value2 = 0
...
with open('PosGenesWithThreshDiff_' + str(thresh) +'.txt', 'w') as f:
