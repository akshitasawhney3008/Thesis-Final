...
with open('new_dict_of_randomsearch_bestparams_rbf40-36.pkl', 'rb') as f:
...
for i in range(iter):
    ...
    C = [x for x in np.linspace(max(0.1, val - 5), min(20, val + 5), num=300)]
    Gamma = [x for x in np.linspace(max(0.001, gval - 0.2), min(1, gval + 0.2), num=100)]
...
with open('ListOfBestParamsGS_40-60.pkl', 'wb') as f:
