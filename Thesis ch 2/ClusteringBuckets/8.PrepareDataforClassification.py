...
def convertGEvaluestobinary(wholedataset,targetcol_train, listOfRangeOfallGenes, gene_names_list):
    ...
    for i in range(wholedataset.shape[1]):
        ...
        print(i)
        list_of_centres = listOfRangeOfallGenes[i]
        gene_col_discrete = []
        for gene_exp in gene_col:
            list_of_diff =[]
            flag = 0
            for centre in list_of_centres:
                absdiff = abs(float(centre) - float(gene_exp))
                list_of_diff.append(absdiff)
            gene_col_discrete.append(str(str(list_of_diff.index(min(list_of_diff))) + gene_names_list[i]))
...
CentresFile = open('CentresFile.txt', 'r').readlines()
listOfCentresOfallGenes =[]
for listOfCentres in CentresFile:
    listOfCentresOfallGenes.append(listOfCentres.rstrip('\n').split(';'))
...
for i in range(iter):
    ...
    gene_col_discrete_arr_train = convertGEvaluestobinary(X_train, Y_train, listOfCentresOfallGenes, gene_names_list)
    gene_col_discrete_arr_test = convertGEvaluestobinary(X_test,Y_test,listOfCentresOfallGenes, gene_names_list)
