...
for i in range(iter):
    file_lasso = open("predictionsProb_lr_nw  " + str(i) + ".csv").readlines()
    file_nw = open("predictionsProb_svmlinear_nw  " + str(i) + ".csv").readlines()
    ...
    np.savetxt('pp_lr_svmlinear_ensemble_chap3' + str(i) + '.csv', ensembles[countlist[ind]-1], delimiter=',')
